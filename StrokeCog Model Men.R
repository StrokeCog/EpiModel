...
#This code generates the model for men
...
#set working directory- folder where model inputs and outputs are stored
setwd("C:/Users/eithnesexton/Desktop/Model Oct 2020")
...
#read in community stroke prevalance data
 ...
 prev_c<-as.data.table(read.xlsx(xlsxFile="P2_1 Stroke Community Prevalence.xlsx",
                              sheet="W3 Stroke Prevalence C M Q",
                              rows=1:7,
                              cols=2:4))
...
#Age groups matching up with QNHS prev_c
...
prev_cogD <- prev_cog[,hesim::rdirichlet_mat(n = 1000, 
                    alpha + c(NCInD_N_C, NCID_Pr_C,CINDnD_Pr_C,CINDD_Pr_C,Dementia_Pr_C)),
                    by="Age.Group"]
...
DMcheckM<-prev[,list(s.3=median(s.3),
                     s.4=median(s.4),
                     s.5=median(s.5),
                     s.6=median(s.6),
                     s.7=median(s.7)
...
write.xlsx(DMcheckM,"DMcheckM.xlsx", rows=1:60,cols=1:6)
...
#### 3. Projections of 40 yr old population ####

popproj<-as.data.table(read.xlsx(xlsxFile="P1_2 Pop Projections.xlsx",
                              sheet="Projections M",
                              rows=1:32,
                              cols=1:2))
...
#need to introduce this reduction gradually - deaths will be underestimated for 2016-2017
...
#values based on utility decrements (1 - weighted score)
#Qol_D = utility for disability state
#QoL_Ac = QoL for the first year post-incident stroke (acute care)

#read in data
utility<-as.data.table(read.xlsx(xlsxFile="P10_Utility.xlsx",
                                  sheet="Utility",
                                  rows=1:2,
                                  cols=1:10))

#PSA based on https://devinincerti.com/2018/02/10/psa.html

#Apply method of moments to mean and sd of utility estimates
#to estimate parameters of the lognormal distribution
#apply separately for utility parameter for each health state

lnorm.pars1<-lnorm_mom(mean = utility$QoL_NCI_m, sd = utility$QoL_NCI_sd)
lnorm.pars2<-lnorm_mom(mean = utility$QoL_CIND_m, sd = utility$QoL_CIND_sd)
lnorm.pars3<-lnorm_mom(mean = utility$QoL_D_m, sd = utility$QoL_D_sd)
lnorm.pars4<-lnorm_mom(mean = utility$QoL_Dem_m, sd = utility$QoL_Dem_sd)
lnorm.pars5<-lnorm_mom(mean = utility$QoL_Ac_m, sd = utility$QoL_Ac_sd)

#prepare parameters for rlnormTrunc function

meanlog1 <- unlist(lnorm.pars1$mu)
sdlog1 <- unlist(sqrt(lnorm.pars1$sigma2))

meanlog2 <- unlist(lnorm.pars2$mu)
sdlog2 <- unlist(sqrt(lnorm.pars2$sigma2))

meanlog3 <- unlist(lnorm.pars3$mu)
sdlog3 <- unlist(sqrt(lnorm.pars3$sigma2))

meanlog4 <- unlist(lnorm.pars4$mu)
sdlog4 <- unlist(sqrt(lnorm.pars4$sigma2))

meanlog5 <- unlist(lnorm.pars5$mu)
sdlog5 <- unlist(sqrt(lnorm.pars5$sigma2))

#generate stochastic estimates for utility for each HS by sampling from the lognormal distribution
#truncated estimate used to avoid values < 0 (as value is calculated as a decrement, cannot be <0)
#estimates plotted to check distribution


n <- 1000
QoL_NCI <- rlnormTrunc(n, meanlog = meanlog1, sdlog = sdlog1, min=0, max = Inf)
QoL_NCI<-as.data.table(QoL_NCI)
QoL_NCI[,iter:=(1:1000)]


ggplot2::ggplot(QoL_NCI, aes_string(x = "QoL_NCI")) +
  geom_density(alpha = .4) + scale_fill_discrete("Distribution") +
  xlab("Simulated values") + ylab("Density")


QoL_CIND <- rlnormTrunc(n, meanlog = meanlog2, sdlog = sdlog2, min=0, max = Inf)
QoL_CIND<-as.data.table(QoL_CIND)
QoL_CIND[,iter:=(1:1000)]

#plots estimates to check distribution
ggplot2::ggplot(QoL_CIND, aes_string(x = "QoL_CIND")) +
  geom_density(alpha = .4) + scale_fill_discrete("Distribution") +
  xlab("Simulated values") + ylab("Density")

QoL_D <- rlnormTrunc(n, meanlog = meanlog3, sdlog = sdlog3, min=0, max = Inf)
QoL_D<-as.data.table(QoL_D)
QoL_D[,iter:=(1:1000)]


ggplot2::ggplot(QoL_D, aes_string(x = "QoL_D")) +
  geom_density(alpha = .4) + scale_fill_discrete("Distribution") +
  xlab("Simulated values") + ylab("Density")


QoL_Dem <- rlnormTrunc(n, meanlog = meanlog4, sdlog = sdlog4, min=0, max = Inf)
QoL_Dem<-as.data.table(QoL_Dem)
QoL_Dem[,iter:=(1:1000)]

ggplot2::ggplot(QoL_Dem, aes_string(x = "QoL_Dem")) +
  geom_density(alpha = .4) + scale_fill_discrete("Distribution") +
  xlab("Simulated values") + ylab("Density")

QoL_Ac <- rlnormTrunc(n, meanlog = meanlog5, sdlog = sdlog5, min=0, max = Inf)
QoL_Ac<-as.data.table(QoL_Ac)
QoL_Ac[,iter:=(1:1000)]

ggplot2::ggplot(QoL_Ac, aes_string(x = "QoL_Ac")) +
  geom_density(alpha = .4) + scale_fill_discrete("Distribution") +
  xlab("Simulated values") + ylab("Density")


#convert from decrement back to utility values

QoL_NCI[,QoL_NCI:=1-QoL_NCI]
QoL_CIND[,QoL_CIND:=1-QoL_CIND]
QoL_D[,QoL_D:=1-QoL_D]
QoL_Dem[,QoL_Dem:=1-QoL_Dem]
QoL_Ac[,QoL_Ac:=1-QoL_Ac]

#check descriptives

mean(QoL_NCI$QoL_NCI)
quantile(QoL_NCI$QoL_NCI, 0.95)
quantile(QoL_NCI$QoL_NCI, 0.05)

mean(QoL_CIND$QoL_CIND)
quantile(QoL_CIND$QoL_CIND, 0.95)
quantile(QoL_CIND$QoL_CIND, 0.05)

mean(QoL_D$QoL_D)
quantile(QoL_D$QoL_D, 0.95)
quantile(QoL_D$QoL_D, 0.05)

mean(QoL_Dem$QoL_Dem)
quantile(QoL_Dem$QoL_Dem, 0.95)
quantile(QoL_Dem$QoL_Dem, 0.05)

mean(QoL_Ac$QoL_Ac)
quantile(QoL_Ac$QoL_Ac, 0.95)
quantile(QoL_Ac$QoL_Ac, 0.05)

#merge with results later on to calculate QALYs
...
#Calculate 40 year old starting prevalences
#all other health states are zero for 40 year olds, as first year in the model 
...
#merges in utility data
res <- merge(res, QoL_NCI, by = c("iter"), allow.cartesian = T )
res <- merge(res, QoL_CIND, by = c("iter"), allow.cartesian = T )
res <- merge(res, QoL_D, by = c("iter"), allow.cartesian = T )
res <- merge(res, QoL_Dem, by = c("iter"), allow.cartesian = T )
res <- merge(res, QoL_Ac, by = c("iter"), allow.cartesian = T )


##calculates model outputs

res[Year>=2015 & Age<90,stroke_deaths:=s.10+s.22+s.35+s.48]
res[Year>=2015 & Age<90,nstroke_deaths:=s.9+s.23+s.36+s.49]
res[Year>=2015 & Age<90,NCI:=s.3+s.4+s.12+s.13+s.17+s.18+s.25+s.26+s.30+s.31
    +s.38+s.39+s.43+s.44]
res[Year>=2015 & Age<90,CIND:=s.5+s.6+s.14+s.15+s.19+s.20+s.27+s.28+s.32+s.33
    +s.40+s.41+s.45+s.46]
res[Year>=2015 & Age<90,Dem:=s.7+s.16+s.21+s.29+s.34+s.42+s.47]

res[Year>=2015 & Age<90,stroke_prev:=s.2+NCI+CIND+Dem+s.11+s.24+s.37]
res[Year>=2015 & Age<90,newCIND:=s.50]
res[Year>=2015 & Age<90,newDem:=s.51]

#stroke_inc = first stroke only
res[Year>=2015 & Age<90,stroke_inc:=s.2]

res[Year>=2015 & Age<90 ,LY:=stroke_prev]
res[Year>=2015 & Age<90,DFLY:=s.2+NCI+CIND+s.11+s.24+s.37]
res[Year>=2015 & Age<90,CIFLY:=s.2+NCI+s.11+s.24+s.37]
res[Year>=2015 & Age<90,Rec:=s.11+s.12+s.13+s.14+s.15+s.16+s.24+s.25
    +s.26+s.27+s.28+s.29+s.37+s.38+s.39+s.40+s.41+s.42]

#QALY calculations

res[Year>=2015 & Age<90,QALY:=(QoL_Ac*(s.2+Rec))+
                      (QoL_NCI*(s.3+s.17+s.30+s.43))+
                       (QoL_D*(s.4+s.6+s.18+s.20+s.31+s.33+s.44+s.46))+
                          (QoL_CIND*(s.5+s.19+s.32+s.45))+
                            (QoL_Dem*Dem)]
...
#sums the individual age groups to give results by year, age group and iteration

x_M<-res[Age<90,list(pop=sum(Pop),stroke_deaths=sum(stroke_deaths),
                   nstroke_deaths=sum(nstroke_deaths),
                   stroke_inc=sum(stroke_inc),
                   NCI=sum(NCI),
                   CIND=sum(CIND),
                   Dem=sum(Dem),
                   newCIND=sum(newCIND),
                   newDem=sum(newDem),
                   stroke_prev=sum(stroke_prev),
                   LY=sum(LY),
                   DFLY=sum(DFLY),
                   CIFLY=sum(CIFLY),
                  Rec=sum(Rec),
                  QALY=sum(QALY)),
             by=list(iter,Year,AgeGroup)]

#sums the individual age groups to give results by year, age group2 and iteration

x_M2<-res[Age<90,list(pop=sum(Pop),stroke_deaths=sum(stroke_deaths),
                     nstroke_deaths=sum(nstroke_deaths),
                     stroke_inc=sum(stroke_inc),
                     NCI=sum(NCI),
                     CIND=sum(CIND),
                     ...
                     Rec=sum(Rec),
                     QALY=sum(QALY)),
         by=list(iter,Year,AgeGroup2)]

#sums the individual age groups to give results by year and iteration

x_allages_M<-res[Age<90,list(pop=sum(Pop),
...
#Calculates the median, and 5th and 95th percentile (uncertainty intervals), for each distribution of outputs

x_M_est<-x_allages_M[Year>=2016,list(pop=max(pop),
...
setcolorder(x_M_est,c("Year","pop","stroke_deathslr","stroke_deathsm","stroke_deathsupr",
                      ...
                      "Reclr","Recm","Recupr",
                      "QALYlr","QALYm","QALYupr"
))

#write out results to excel
write.xlsx(x_M_est,"M_est_BC.xlsx", rows=1:30,cols=1:46)


#results by age

x_M_est_age<-x_M[Year>=2016,list(pop=max(pop),
                                     stroke_deathslr=quantile(stroke_deaths, 0.05),
                                     nstroke_deathslr=quantile(nstroke_deaths, 0.05),
                                     stroke_inclr=quantile(stroke_inc, 0.05),
                                     NCIlr=quantile(NCI, 0.05),
...
by=list(Year,AgeGroup)]


#set column order
setcolorder(x_M_est_age,c("Year","pop","stroke_deathslr","stroke_deathsm","stroke_deathsupr",
                      "nstroke_deathslr", "nstroke_deathsm", "nstroke_deathsupr",
                      "stroke_inclr","stroke_incm","stroke_incupr",
                      "NCIlr", "NCIm", "NCIupr",
                      "CINDlr","CINDm","CINDupr",
...
#Output to Excel
write.xlsx(x_M_est_age,"M_est_age_BC.xlsx", rows=1:100,cols=1:46)


#results by age group 2

x_M_est_age2<-x_M2[Year>=2016,list(pop=max(pop),
                                 stroke_deathslr=quantile(stroke_deaths, 0.05),
                                 nstroke_deathslr=quantile(nstroke_deaths, 0.05),
                                 stroke_inclr=quantile(stroke_inc, 0.05),
                                 NCIlr=quantile(NCI, 0.05),
...
#set column order
setcolorder(x_M_est_age2,c("Year","pop","stroke_deathslr","stroke_deathsm","stroke_deathsupr",
                          "nstroke_deathslr", "nstroke_deathsm", "nstroke_deathsupr",
                          "stroke_inclr","stroke_incm","stroke_incupr",
                          "NCIlr", "NCIm", "NCIupr",
                          "CINDlr","CINDm","CINDupr",
...
#Output to Excel
write.xlsx(x_M_est_age2,"M_est_age2_BC.xlsx", rows=1:100,cols=1:46)

####18. Validation ####

#code validation check moves add up to health states? 

#validation against CSO mortality data


#select out equivalent model predictions - only up to age 84 to align with CSO age bands
res[Age>=40 & Age<65, AgeGroup3:="40-64"]
res[Age>=65 & Age<75, AgeGroup3:="65-74"]
res[Age>=75 & Age<85, AgeGroup3:="75-84"]
res[Age>=85, AgeGroup3:="85+"]



#sums the individual age groups to give results for stroke deaths by year, age group 3 and iteration

sd_age<-res[Year>2015&Age<85&Year<2018,list(pop=sum(Pop),stroke_deaths=sum(stroke_deaths)),
            by=list(iter,Year,AgeGroup3)]

#results by age group 3
sd_M_est_age<-sd_age[,list(stroke_deathsm=median(stroke_deaths),
                           stroke_deathslr=quantile(stroke_deaths, 0.05),
                           stroke_deathsupr=quantile(stroke_deaths, 0.95)
),
by=list(Year,AgeGroup3)]

#sums the individual age groups to give results for stroke deaths for all ages by year and iteration

sd<-res[Year>2015&Age<85&Year<2018,list(pop=sum(Pop),stroke_deaths=sum(stroke_deaths)),
        by=list(iter,Year)]

#results by age group 3
sd_M_est<-sd[,list(stroke_deathsm=median(stroke_deaths),
                   stroke_deathslr=quantile(stroke_deaths, 0.05),
                   stroke_deathsupr=quantile(stroke_deaths, 0.95)
),
by=list(Year)]
